...
def test_11_codex_doctor(repo, gh, tmp_path, monkeypatch):
    ...
    # The one required UI skill, where the Codex worker reads skills; test_29 covers its absence.
    (codex_home / "skills" / "impeccable").mkdir(parents=True)
    (codex_home / "skills" / "impeccable" / "SKILL.md").write_text("impeccable\n", encoding="utf-8")
