...
def _gate_content_hash(path: Path) -> str:
    """Hash a gate file's content, line-ending-INDEPENDENT.

    The manifest is generated by `forge upgrade`/`adopt` (often against a
    Windows working tree, so CRLF) and verified on Linux CI (LF). Hashing raw
    bytes made every gate file read as drift across platforms — the whole gate
    surface would fail `project audit` right after a Windows re-vendor. Normalise
    CRLF->LF first so a client's manifest matches regardless of how Git checked
    the files out. The gate surface is all text (scripts/schemas/prompts/config);
    there are no binary gate files.
    """
    ...
    for tree in GATE_TREES:
        ...
        for path in sorted(root.rglob("*")):
            if path.is_file() and "__pycache__" not in path.parts \
                ...
                hashes[rel] = _gate_content_hash(path)
