...
| save and approve a plan | `python3 factory/scripts/forge.py plan save --from <plan-file> --story <key>` records `awaiting-approval`; after one independent cold grill and complete disposition/amendment bridge, show the exact final plan in native Plan Mode. Successful Claude `ExitPlanMode` binds its exact plan input; Codex uses id `approve_plan_<digest>`, prompt `Approve exact plan digest <digest>?`, and an id-keyed `Approve plan` answer |
