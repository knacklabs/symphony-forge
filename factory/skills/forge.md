...
| save and approve a plan | Commit any spec, decision, or roadmap changes before launching the cold grill because its staleness check compares commit order. Run one independent wide cold grill against the draft and complete its disposition/amendment bridge; before saving, review every active decision, then run `python3 factory/scripts/forge.py plan save --from <plan-file> --story <key>` once. `forge plan save` records that review attestation in `.factory/stories/<story>/plan-meta.json`. Show the story plan body exactly as saved in native Plan Mode, without Forge bookkeeping in the displayed brief. An approved story amendment needs fresh native approval only if `What changes for you` or `Done when` changes; otherwise the recorder carries approval forward with a reason. Task plans need a matching recorded cold read, without human approval. Successful Claude `ExitPlanMode` binds its exact plan input; Codex retains id `approve_plan_<digest>`, prompt `Approve this plan?`, and an id-keyed `Approve plan` answer as recorder details; the digest travels only in the question id. |
