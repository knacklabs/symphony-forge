...
The review runs inside the reviewed worktree, read-only. `forge review`
writes a small launcher under the forge control dir
(`.git/forge/review-launcher/<task>/`) and hands it to the skill as its Codex
binary; the launcher replaces the skill's empty working folder with the
...
- The host's triage (0075) verifies rather than discovers: the reviewer's
