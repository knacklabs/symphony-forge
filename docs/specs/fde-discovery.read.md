...
1. The office-hours requirement conflicts with an accepted decision.
   [Decision 0089](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/docs/decisions/0089-fde-discovery-without-gstack.md:29) says gstack is removed and Forge discovery needs no third-party bundle. The spec requires `/office-hours` for new projects and big ideas. The decision needs an explicit amendment before approval.
...
3. The problem-card destination is unresolved.
   The existing [DISCOVERY.md template](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/src/forge/templates/skeleton/docs/product/DISCOVERY.md:5) has `## Problem`, while the spec adds `## Problems`; the [Brief template](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/src/forge/templates/skeleton/docs/product/BRIEF.md:5) has no problem field. Specify where existing problem text goes, where the card heading goes in the Brief, and the six field labels used by each card.
...
4. Reading context notes directly contradicts the current inbox rule.
   [docs/context/README.md](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/docs/context/README.md:9) requires ledger registration and harvesting rather than ad hoc reading. The spec excludes `forge context mark` while telling the agent to read notes and populate cards. It must reconcile that workflow and its CI ledger check.
...
7. The success-check lifecycle conflicts with the confirmed-body lock.
   [spec confirmation](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/src/forge/records.py:145) stores a hash of the body, and [roadmap add](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/src/forge/records.py:213) refuses a changed body. Appending `Result` changes that body, so the spec’s promise that a later story can be added needs a defined hash and reconfirmation rule.
...
8. “Every roadmap story is done” needs a source of truth.
   Roadmap items have a status, but [story done](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/src/forge/story.py:170) records completion in story state without updating the roadmap item. Specify which record `forge next` reads, and whether a spec with no linked stories can become due.
...
11. Cut or defer: the impeccable requirement in this spec.
    The [current Forge skill](/Users/ravikiranvemula/Workdir/symphony-forge-fix-the-fde-discovery-spec-still-says-gstack/src/forge/templates/skill.md:82) already makes impeccable the UI skill. The spec’s Why and success measure do not call for another UI-tool change.
