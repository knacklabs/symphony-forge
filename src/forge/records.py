...
Every command here refuses the default branch before writing, then commits what it wrote on
the current branch (a fix branch, normally), so the change reaches main by `forge close`.
...
REFUSALS = {
    ...
    "spec_confirmed": ("docs/specs/{slug}.md is already confirmed; saving it again would undo the "
                       "human's confirmation.", "forge spec save <new-slug>"),
    ...
    "roadmap_section": ("The Roadmap section of docs/specs/{slug}.md needs one `- KEY: title` line "
                        "per story: {problem}.", "forge roadmap add {slug}"),
...
def spec_confirm(args: argparse.Namespace) -> None:
    ...
    rel, text, fields, _ = _spec(top, args.slug)
...
def decision_new(args: argparse.Namespace) -> None:
    top = _start(args, args.slug)
    existing = _decision(top, args.slug)
    if existing:
...
def decision_accept(args: argparse.Namespace) -> None:
    top = _start(args, args.slug)
    by = _name(args, args.slug)
    rel = _decision(top, args.slug)
    if not rel:
        repo.refuse(REFUSALS["no_decision"], slug=args.slug)
    text = _text(top, rel)
    fields, _ = _front(text)
    ...
    if old:
        # The frontmatter names the record it replaces by slug or by NNNN-slug.
        old_rel = _decision(top, re.sub(r"^\d{4}-", "", old))
...
def roadmap_add(args: argparse.Namespace) -> None:
    ...
    for line in _sections(body).get("Roadmap", "").strip().splitlines():
        ...
        if not match or match[1] in wanted:
...
def _start(args: argparse.Namespace, slug: str) -> Path:
    """The checkout's top, once the slug is sound and the branch may change."""
...
def _decision(top: Path, slug: str) -> str:
    """The repo path of the decision with this slug, or ""."""
    if not SLUG.fullmatch(slug):
        return ""
    found = sorted((top / "docs" / "decisions").glob(f"[0-9][0-9][0-9][0-9]-{slug}.md"))
    return f"docs/decisions/{found[-1].name}" if found else ""
...
def _set(text: str, **changes: str) -> str:
    """The doc with these frontmatter fields set; its other lines stay as they are."""
    match = FRONTMATTER.match(text)
