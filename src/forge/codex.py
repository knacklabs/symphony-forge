...
# Loads the SDK and finds its bundled Codex program (that call raises when the program is missing),
# then prints both versions: the SDK pins its Codex program to its own version.
PROBE = ("import importlib.metadata as m, openai_codex, codex_cli_bin; "
         "codex_cli_bin.bundled_codex_path(); "
         "print(*(f'{name} {m.version(name)}' "
         "for name in ('openai-codex', 'openai-codex-cli-bin')), sep=', ')")
WANTED = f"openai-codex {SDK_PIN}, openai-codex-cli-bin {SDK_PIN}"
...
def sdk_problem() -> str:
    ...
    if done.returncode or said != WANTED:
